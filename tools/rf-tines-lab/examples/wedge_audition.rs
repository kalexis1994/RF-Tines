//! How much would the wedge actually change the sound?
//!
//! The fit reports harmonic balance error, which is not a loudness and not a
//! perception. This renders the shipping voicing and the wedge candidate side
...
//!
//! `cargo run --release -p rf-tines-lab --example wedge_audition -- OUT_DIR`
#[allow(dead_code)]
#[path = "matts_fit.rs"]
mod previous;
...
fn main() -> Result<()> {
    ...
    for (note, name) in NOTES {
        for (velocity, layer) in LAYERS {
            ...
            write_wav(&out.join(format!("{name}-{layer}-cuna.wav")), &candidate)?;
        }
    }
    println!(
